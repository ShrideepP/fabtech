...
import { finalisedMeasurementsSchema as formSchema } from "@/lib/schemas/process-details";
...
export default function ProcessDetails({
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      router.push("/finalised-measurements");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function getBillFiles() {
    setIsBillLoading(true);
    try {
      const { data, error } = await getFiles(id as string, FOLDER_NAMES.bill);
  ...
  async function handleBillChange(event: React.ChangeEvent<HTMLInputElement>) {
    if (event.target.files) {
      try {
        await handleUpload(FOLDER_NAMES.bill, event.target.files[0]);
      } finally {
        getBillFiles();
      }
    }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <FileUpload
            path={FOLDER_NAMES.bill}
            files={bill}
            isLoading={isBillLoading}
            refetchFiles={getBillFiles}
            mode={mode === "read" ? "read" : "createOrUpdate"}
        ...
        {mode !== "read" && (
          <div className="flex items-center gap-2.5">
            ...
            <Link href="/finalised-measurements">
